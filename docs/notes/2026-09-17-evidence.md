...
| NMS 最差帧（间隔 ≥ 1 s） | 544, 479, 594, 1079, 779 | 61, 135, 11, 185, 235 |
| bounds overflow | 0.204；出界维 `[1,4,5]` | 0.170；出界维 `[1,4]` |
| tracking | median corr 0.083；std 比 0.138 | median corr 0.064 |
| 相机 PNG | 5 帧 × 4 路（去重后仍 5 个峰） | 5 帧 × 4 路 |
...
Pearson corr 对正仿射不变，所以换统计量后 corr 逐维一样是数学必然，不是发现。实验新增的只有 L2 2.58 → 1.73：大约三分之一是均值错位，其余是形状不跟。判定仍是**权重主因**（模型卡写的训练集是 `aloha_mobile_wash_pan`），统计量贴错是叠加层。没再跑 10 集 `tracking_judge`。

B' 在同一 ep0 上 median corr 0.825、std 比 1.14、L2 1.348。正对照会跟，但 d3 corr −0.84、d4 −0.49，最难一集也不干净。

图顶是逐帧 L2（全程贴在门槛之上，不是尖峰）。bounds 改成虚线，出界点标红。竖线是 NMS 后间隔 ≥1 s 的峰，不再是 544 附近连拍。
